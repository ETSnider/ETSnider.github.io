...
def sumVersions(signal):
	...
	if typeID == '100':
		...
		for subpacket in range(numSubpackets):
			tempa,tempb = sumVersions(signal[readPosition:])
			versionNum += tempa
			readPosition += tempb
			
		return versionNum, readPosition
	else:
		return versionNum, readPosition + readPosition%4

def main():
	###First, read and parse the input signal from file.
	signal = getInput()
	###Next, find the sum of the version numbers of the packets.
	versionSum, readPosition = sumVersions(signal)
