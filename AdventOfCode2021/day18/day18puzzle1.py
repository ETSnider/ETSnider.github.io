...
Given a list of snailfish numbers, you must add them all up in the order they appear, and find the result's magnitude.
"""
import sys
...
def main():
	snailNums = getInput()
	snailSum  = to_AST(eval(snailNums[0]))
	#print(snailSum)
	#print(len(snailNums))
	for line in range(1,len(snailNums)):
		snailSum = addSnailNums(snailSum,to_AST(eval(snailNums[line])))
		#print(snailSum)
