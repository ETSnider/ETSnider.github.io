...
"""
Advent of Code
Day 4 Challenge Part 2

Task: 
Given a series of bingo numbers, and a list of bingo boards, determine which board would win last.
...
def main():
    ...
    ####Remove won boards until the last board, then print its score to the console when it wins.
	callnum = 0
	cards = len(bingoBoards)
	while cards > 0:
		numcall = bingoNums[callnum]
		...
		if cards ==1:
			lastcall=numcall
			last=bingoBoards[0]
		bingoBoards = list(x for x in bingoBoards if checkWin(x)==-1)
		cards = len(bingoBoards)
		callnum += 1
	
	print(lastcall)
	print(last)
	print(lastcall*calcScore(last))
